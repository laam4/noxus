ALTER TABLE `geoLiteCity_Location` 
CHANGE `country` `country` varchar(2) NOT NULL default '';

ALTER TABLE `hlstats_Actions`
CHANGE `game` `game` varchar(64) NOT NULL default 'valve',
CHANGE `code` `code` varchar(64) NOT NULL default '',
CHANGE `team` `team` varchar(64) NOT NULL default '',
CHANGE `object` `object` varchar(64) NOT NULL default '',
CHANGE `event` `event` varchar(64) NOT NULL default '',
DROP KEY `code`,
...
ALTER TABLE hlstats_Options DROP PRIMARY KEY;
ALTER TABLE hlstats_Options DROP COLUMN `game`;
ALTER TABLE hlstats_Options DROP COLUMN `key_id`;
ALTER TABLE hlstats_Options ADD PRIMARY KEY (`keyname`);
...
CHANGE `uniqueId` `uniqueId` varchar(128) NOT NULL default '';
...
ALTER TABLE `hlstats_Servers` DROP COLUMN `sortorder`;

UPDATE `hlstats_Servers_Config_Default` SET `parameter`='HLStatsURL' WHERE `parameter`='HLstatsURL';
